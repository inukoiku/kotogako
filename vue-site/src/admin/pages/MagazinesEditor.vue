        ...
        <template #item="{ element }">
          <div class="item-card" :class="{ inactive: !element.active }">
            ...
            <div class="item-info">
              ...
              <p class="item-desc">{{ element.kicker }}</p>
              <div class="item-meta">
            ...
            <div class="item-actions">
              <router-link
                class="btn-icon"
                :to="`/admin/magazines/${element.id}/pages`"
                title="管理頁面"
              >
                📄
              </router-link>
          ...
          <form @submit.prevent="saveItem" class="modal-form">
            <div class="form-row">
              <div class="form-group">
                <label>雜誌標題 *</label>
                <input v-model="formData.title" type="text" placeholder="犬高雜誌-第二期" required />
              </div>
              <div class="form-group">
                <label>排序</label>
            ...
            <div class="form-group">
              <label>期數標籤</label>
              <input v-model="formData.kicker" type="text" placeholder="Vol. 02" />
            </div>

            <div class="form-group">
              <label>封面圖片網址 *</label>
              <input v-model="formData.coverUrl" type="url" placeholder="https://..." required />
              <div v-if="formData.coverUrl" class="image-preview">
                <img :src="formData.coverUrl" alt="預覽" />
              </div>
            </div>
      ...
      <div v-if="showDeleteModal" class="modal-overlay" @click="showDeleteModal = false">
        <div class="modal-content modal-small" @click.stop>
          ...
          <p class="delete-message">
            確定要刪除「{{ currentItem?.title }}」嗎？此操作不會自動刪除底下的頁面資料，此操作無法復原。
          </p>
          <div class="modal-actions">
...
import { useMagazinesEditor } from '@/admin/composables/useMagazinesEditor';
